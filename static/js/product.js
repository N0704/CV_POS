class ProductSystem {
        ...
        tbody.innerHTML = products.map((p, index) => `
            <tr class="odd:bg-white even:bg-neutral-50">
                ...
                <td class="px-4 py-3 flex justify-center items-center gap-1.5">
                        <i onclick="productSystem.showUpdateProduct(${p.id})" data-lucide="square-pen" class="w-4 h-4 cursor-pointer text-neutral-800 hover:opacity-60"></i>
                        <i onclick="productSystem.deleteProduct(${p.id})" data-lucide="trash" class="w-4 h-4 cursor-pointer text-primary hover:opacity-60"></i>
                </td>
            </tr>
        `).join("");
    ...
    async addProduct(e) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                this.showToast('Lỗi', result.error || 'Không thể thêm sản phẩm', 'error');
            }
        } catch (error) {
    ...
    async showUpdateProduct(productId) {
        try {
            ...
            const product = await this.request(`/product/${productId}`, { method: "GET" });
    ...
    async submitUpdateProduct(e) {
        ...
        try {
            ...
            if (res.ok) {
                ...
                this.showToast('Lỗi', result.error || 'Không thể cập nhật sản phẩm', 'error');
            }
        } catch (error) {
    ...
    async deleteProduct(productId) {
        ...
        try {
            ...
            if (res.ok) {
                this.showToast('Thành công', result.message || 'Đã xóa sản phẩm', 'success');
                this.loadProduct();
            } else {
                this.showToast('Lỗi', result.error || 'Không thể xóa sản phẩm', 'error');
            }
        } catch (error) {
    ...
    setupEventListeners() {         
        ...
        if (newProductBtn) {
            newProductBtn.addEventListener("click", () => this.showModal("newProductModal"));
        }
        ...
        if (createBtn) createBtn.addEventListener("submit", (e) => this.addProduct(e));

        const updateBtn = document.getElementById("updateProductForm");
        if (updateBtn) updateBtn.addEventListener("submit", (e) => this.submitUpdateProduct(e));
    }
}
