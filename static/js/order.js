        ...
        (o, index) => `
            <tr class="odd:bg-white even:bg-neutral-50">
                ...
                <td class="px-4 py-3 flex justify-center hover:opacity-60">
                        <i onclick="orderSystem.showOrderDetails('${
                          o.id
                        }')" data-lucide="ellipsis" class="w-4 h-4 cursor-pointer"></i>
                </td>
            </tr>
        `
      )
  ...
  async showOrderDetails(orderId) {
    try {
                    ...
                    <tr class="text-sm text-neutral-800 text-center hover:bg-neutral-50 transition-colors">
                        <td class="px-8 py-4 text-left w-1/2">
                            <div>
                                ...
                                <p class="text-neutral-500 text-xs mt-1">Mã: ${
                                  item.barcode || "-"
                                }</p>
                            </div>
                        </td>
                        <td class="px-4 py-4 w-1/6">${this.formatCurrency(
                          item.price
                        )}₫</td>
      ...
      const totalPaymentEl = document.querySelector(
        "#orderModal .totalPayment"
      );
      const grandTotalEl = document.querySelector("#orderModal .grandTotal");
      if (totalPaymentEl)
        totalPaymentEl.innerText = this.formatCurrency(total) + "₫";
      if (grandTotalEl)
        grandTotalEl.innerText = this.formatCurrency(total) + "₫";
  ...
  renderCartRow(barcode, item) {
    return `
        <tr class="text-sm text-neutral-800 hover:bg-neutral-50 transition-colors">
            ...
            <td class="px-4 py-4 text-center">${this.formatCurrency(
              item.price
            )}₫</td>
            <td class="px-4 py-3 text-center">
                <div class="flex items-center justify-center gap-1">
                    <button class="w-6 h-6 flex items-center justify-center border border-neutral-300 rounded bg-white hover:bg-neutral-100"
                        onclick="orderSystem.updateQty('${barcode}', ${
      item.qty - 1
    })">-</button>
      ...
      item.qty + 1
            ...
            <td class="px-4 py-4 text-center font-semibold">${this.formatCurrency(
              item.total
            )}₫</td>
            <td class="px-4 py-3 text-center">
    ...
    try {
      ...
      tempDiv.innerHTML = `
                ...
                <div style="margin-bottom: 20px;">
                    ...
                    <p style="margin: 5px 0;"><strong>Ngày đặt hàng:</strong> ${
                        ...
                        ${data.items
                          .map(
                            ...
                            <tr>
                                ...
                                <td style="border: 1px solid #000; padding: 8px; text-align: right;">${this.formatCurrency(
                                  item.price
                                )}₫</td>
                                <td style="border: 1px solid #000; padding: 8px; text-align: right; font-weight: bold;">${this.formatCurrency(
                                  item.total
                                )}₫</td>
                            </tr>
                        `
  ...
  async checkout() {
    ...
    if (result?.order_id) {
      this.showToast(
        ...
        `Đơn hàng ${result.order_id} đã được tạo!`,
        "success"
      );
